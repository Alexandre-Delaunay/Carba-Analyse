...
BOOL kull_m_token_getNameDomainFromToken(HANDLE hToken, PWSTR * pName, PWSTR * pDomain, PWSTR * pSid, PSID_NAME_USE pSidNameUse)
{
	...
	{
		if(pTokenUser = (PTOKEN_USER) API(KERNEL32, LocalAlloc)(LPTR, szNeeded))
		{
			...
			API(KERNEL32, LocalFree)(pTokenUser);
		}
	}
	return result;
}
...
BOOL kull_m_token_getNameDomainFromSID(PSID pSid, PWSTR * pName, PWSTR * pDomain, PSID_NAME_USE pSidNameUse)
{
	...
	{
		if(*pName = (PWSTR) API(KERNEL32, LocalAlloc)(LPTR, cchName * sizeof(wchar_t)))
		{
			if(*pDomain = (PWSTR) API(KERNEL32, LocalAlloc)(LPTR, cchReferencedDomainName * sizeof(wchar_t)))
			{
				result = LookupAccountSid(NULL, pSid, *pName, &cchName, *pDomain, &cchReferencedDomainName, peUse);
				if(!result)
					*pDomain = (PWSTR) API(KERNEL32, LocalFree)(*pDomain);
			}
			if(!result)
				*pName = (PWSTR) API(KERNEL32, LocalFree)(*pName);
		}
	}
	return result;
}
...
BOOL CALLBACK kull_m_token_getTokens_process_callback(PSYSTEM_PROCESS_INFORMATION pSystemProcessInformation, PVOID pvArg)
{
	...
	if(hProcess = API(KERNEL32, OpenProcess)(PROCESS_QUERY_INFORMATION, FALSE, (ULONG) pSystemProcessInformation->UniqueProcessId))
	{
...
BOOL CALLBACK kull_m_token_getTokens_handles_callback(PSYSTEM_HANDLE pSystemHandle, PVOID pvArg)
{
	...
	if(hProcess = API(KERNEL32, OpenProcess)(PROCESS_DUP_HANDLE, FALSE, pSystemHandle->ProcessId))
	{
		if(DuplicateHandle(hProcess, (HANDLE) pSystemHandle->Handle, API(KERNEL32, GetCurrentProcess)(), &hRemoteHandle, TOKEN_QUERY | TOKEN_DUPLICATE, TRUE, 0))
		{
			if(NtQueryObject(hRemoteHandle, ObjectTypeInformation, NULL, 0, &szNeeded) == STATUS_INFO_LENGTH_MISMATCH)
			{
				if(pInfos = (POBJECT_TYPE_INFORMATION) API(KERNEL32, LocalAlloc)(LPTR, szNeeded))
				{
					if(NT_SUCCESS(NtQueryObject(hRemoteHandle, ObjectTypeInformation, pInfos, szNeeded, &szNeeded)))
						if(API(NTDLL, RtlEqualUnicodeString)(&pInfos->TypeName, &kull_m_token_strToken, TRUE))
							status = ((PKULL_M_TOKEN_ENUM_DATA) pvArg)->callback(hRemoteHandle, pSystemHandle->ProcessId, ((PKULL_M_TOKEN_ENUM_DATA) pvArg)->pvArg);
					API(KERNEL32, LocalFree)(pInfos);
				}
			}
			API(KERNEL32, CloseHandle)(hRemoteHandle);
		}
		API(KERNEL32, CloseHandle)(hProcess);
	}
	((PKULL_M_TOKEN_ENUM_DATA) pvArg)->mustContinue = status;
