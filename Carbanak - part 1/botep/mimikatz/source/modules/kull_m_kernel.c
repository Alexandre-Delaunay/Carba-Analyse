...
BOOL kull_m_kernel_ioctl(PCWSTR driver, DWORD ioctlCode, PVOID bufferIn, DWORD szBufferIn, PVOID * pBufferOut, PDWORD pSzBufferOut)
{
	...
	{
		for(*pSzBufferOut = 0x10000; (lStatus == ERROR_MORE_DATA) && (*pBufferOut = API(KERNEL32, LocalAlloc)(LPTR, *pSzBufferOut)) ; *pSzBufferOut <<= 1)
		{
			...
			else
			{
				...
				API(KERNEL32, LocalFree)(*pBufferOut);
			}
		}
		if(lStatus)
		{
		...
		API(KERNEL32, CloseHandle)(hDriver);
	}
