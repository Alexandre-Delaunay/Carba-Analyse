...
BOOL kull_m_patch(PKULL_M_MEMORY_SEARCH sMemory, PKULL_M_MEMORY_ADDRESS pPattern, SIZE_T szPattern, PKULL_M_MEMORY_ADDRESS pPatch, SIZE_T szPatch, LONG offsetOfPatch, PKULL_M_PATCH_CALLBACK pCallBackBeforeRestore, int argc, wchar_t * args[], NTSTATUS * pRetCallBack)
{
	...
	if(kull_m_memory_search(pPattern, szPattern, sMemory, TRUE))
	{
		...
		if(!resultBackup)
			if(backup.address = API(KERNEL32, LocalAlloc)(LPTR, szPatch))
		...
		if(resultBackup)
		{
			...
			if(backup.address)
				API(KERNEL32, LocalFree)(backup.address);
		}
	}
	return result;
}
	...
	if(currenReferences = kull_m_patch_getGenericFromBuild(generics, cbGenerics, MIMIKATZ_NT_BUILD_NUMBER))
	{
		...
		if( isService )
		{
			if(kull_m_service_getUniqueForName(processOrService, &ServiceStatusProcess))
			{
				...
				{
					pid = ServiceStatusProcess.dwProcessId;
				}
				else
					PRINT_ERROR(L"Service is not running\n");
			}
			else
				PRINT_ERROR_AUTO(L"kull_m_service_getUniqueForName");
		}
		else
			pid = (DWORD)processOrService;
		if(hProcess = API(KERNEL32, OpenProcess)( PROCESS_VM_READ | PROCESS_VM_WRITE | PROCESS_VM_OPERATION | PROCESS_QUERY_INFORMATION, FALSE, pid))
		{
			if(kull_m_memory_open(KULL_M_MEMORY_TYPE_PROCESS, hProcess, &hMemory))
			{
				if(kull_m_process_getVeryBasicModuleInformationsForName(hMemory, moduleName, &iModule))
				{
					...
					do
					{
						...
						{
							kprintf(_WCS_("\"%s\" service patched\n"), processOrService);
							result = TRUE;
						}
						else
						{
						...
						currenReferences--;
					} while( currenReferences >= generics && currenReferences->MinBuildNumber == currenReferences[1].MinBuildNumber );
