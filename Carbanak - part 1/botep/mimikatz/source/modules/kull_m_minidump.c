...
BOOL kull_m_minidump_open(IN HANDLE hFile, OUT PKULL_M_MINIDUMP_HANDLE *hMinidump)
{
	...
	*hMinidump = (PKULL_M_MINIDUMP_HANDLE) API(KERNEL32, LocalAlloc)(LPTR, sizeof(KULL_M_MINIDUMP_HANDLE));
	if(*hMinidump)
	{
		(*hMinidump)->hFileMapping = API(KERNEL32, CreateFileMappingW)(hFile, NULL, PAGE_READONLY, 0, 0, NULL);
		if((*hMinidump)->hFileMapping)
		{
			if((*hMinidump)->pMapViewOfFile = API(KERNEL32, MapViewOfFile)((*hMinidump)->hFileMapping, FILE_MAP_READ, 0, 0, 0))
...
BOOL kull_m_minidump_close(IN PKULL_M_MINIDUMP_HANDLE hMinidump)
{
	if(hMinidump->pMapViewOfFile)
		API(KERNEL32, UnmapViewOfFile)(hMinidump->pMapViewOfFile);
	if(hMinidump->hFileMapping)
		API(KERNEL32, CloseHandle)(hMinidump->hFileMapping);
	return TRUE;
}
