...
NTSTATUS kull_m_process_getProcessInformation(PKULL_M_PROCESS_ENUM_CALLBACK callBack, PVOID pvArg)
{
	...
	for(sizeOfBuffer = 0x1000; (status == STATUS_INFO_LENGTH_MISMATCH) && (buffer = (PSYSTEM_PROCESS_INFORMATION) API(KERNEL32, LocalAlloc)(LPTR, sizeOfBuffer)) ; sizeOfBuffer <<= 1)
	{
		status = API(NTDLL, NtQuerySystemInformation)(SystemProcessInformation, buffer, sizeOfBuffer, NULL);
...
{
	if(((PKULL_M_PROCESS_PID_FOR_NAME) pvArg)->isFound = API(NTDLL, RtlEqualUnicodeString)(&pSystemProcessInformation->ImageName, ((PKULL_M_PROCESS_PID_FOR_NAME) pvArg)->name, TRUE))
...
BOOL kull_m_process_getProcessIdForName(LPCWSTR name, PDWORD processId)
{
	...
	API(NTDLL, RtlInitUnicodeString)(&uName, name);
			...
			if(kull_m_memory_copy(&aBuffer, &aProcess, sizeof(LdrData)))
			{
					...
					)
				{
					...
					if(continueCallback = kull_m_memory_copy(&aBuffer, &aProcess, sizeof(LdrEntry)))
					{
						...
						if(moduleName.Buffer = (PWSTR) API(KERNEL32, LocalAlloc)(LPTR, moduleName.MaximumLength))
						{
							...
							API(KERNEL32, LocalFree)(moduleName.Buffer);
						}
					}
				}
				status = STATUS_SUCCESS;
	...
	case KULL_M_MEMORY_TYPE_PROCESS_DMP:
		...
		if(pMinidumpModuleList = (PMINIDUMP_MODULE_LIST) kull_m_minidump_stream(memory->pHandleProcessDmp->hMinidump, ModuleListStream))
		{
			for(i = 0; (i < pMinidumpModuleList->NumberOfModules) && continueCallback; i++)
			{
				...
				{
					API(NTDLL, RtlInitUnicodeString)(&moduleName, wcsrchr(pMinidumpString->Buffer, L'\\') + 1);
					continueCallback = callBack(&moduleInformation, pvArg);
				}
			}
			status = STATUS_SUCCESS;
...
{
	if(((PKULL_M_PROCESS_VERY_BASIC_MODULE_INFORMATION_FOR_NAME) pvArg)->isFound = API(NTDLL, RtlEqualUnicodeString)(pModuleInformation->NameDontUseOutsideCallback, ((PKULL_M_PROCESS_VERY_BASIC_MODULE_INFORMATION_FOR_NAME) pvArg)->name, TRUE))
...
NTSTATUS kull_m_process_getMemoryInformations(PKULL_M_MEMORY_HANDLE memory, PKULL_M_MEMORY_RANGE_ENUM_CALLBACK callBack, PVOID pvArg)
{
	...
	if(!NT_SUCCESS(API(NTDLL, NtQuerySystemInformation)(KIWI_SystemMmSystemRangeStart, &maxPage, sizeof(PBYTE), NULL)))
	...
	case KULL_M_MEMORY_TYPE_OWN:
		for(currentPage = 0; (currentPage < maxPage) && continueCallback; currentPage += memoryInfos.RegionSize)
			if(API(KERNEL32, VirtualQuery)(currentPage, &memoryInfos, sizeof(MEMORY_BASIC_INFORMATION)) == sizeof(MEMORY_BASIC_INFORMATION))
	...
	case KULL_M_MEMORY_TYPE_PROCESS:
		for(currentPage = 0; (currentPage < maxPage) && continueCallback; currentPage += memoryInfos.RegionSize)
			if(API(KERNEL32, VirtualQueryEx)(memory->pHandleProcess->hProcess, currentPage, &memoryInfos, sizeof(MEMORY_BASIC_INFORMATION)) == sizeof(MEMORY_BASIC_INFORMATION))
				continueCallback = callBack(&memoryInfos, pvArg);
			else break;
		status = STATUS_SUCCESS;
		break;
...
BOOL kull_m_process_peb(PKULL_M_MEMORY_HANDLE memory, PPEB pPeb, BOOL isWOW)
{
	...
	HANDLE hProcess = (memory->type == KULL_M_MEMORY_TYPE_PROCESS) ? memory->pHandleProcess->hProcess : API(KERNEL32, GetCurrentProcess)();
	KULL_M_MEMORY_HANDLE  hBuffer = {KULL_M_MEMORY_TYPE_OWN, NULL};
	...
	case KULL_M_MEMORY_TYPE_OWN:
		if(!isWOW)
		{
			*pPeb = *API(NTDLL, RtlGetCurrentPeb)();
			status = TRUE;
			break;
		}
	case KULL_M_MEMORY_TYPE_PROCESS:
		if(NT_SUCCESS(API(NTDLL, NtQueryInformationProcess)(hProcess, info, buffer, szBuffer, &szInfos)) && (szInfos == szBuffer) && processInformations.PebBaseAddress)
		{
...
BOOL kull_m_process_ntheaders(PKULL_M_MEMORY_ADDRESS pBase, PIMAGE_NT_HEADERS * pHeaders)
{
	...
	if(kull_m_memory_copy(&aBuffer, pBase, sizeof(IMAGE_DOS_HEADER)) && headerImageDos.e_magic == IMAGE_DOS_SIGNATURE)
	{
		...
		if(aBuffer.address = API(KERNEL32, LocalAlloc)(LPTR, sizeof(DWORD) + IMAGE_SIZEOF_FILE_HEADER))
		{
			if(kull_m_memory_copy(&aBuffer, &aProcess, sizeof(DWORD) + IMAGE_SIZEOF_FILE_HEADER) && ((PIMAGE_NT_HEADERS) aBuffer.address)->Signature == IMAGE_NT_SIGNATURE);
			{
				...
				if(aRealNtHeaders.address = (PIMAGE_NT_HEADERS) API(KERNEL32, LocalAlloc)(LPTR, size))
				{
					...
					if(status)
						*pHeaders = (PIMAGE_NT_HEADERS) aRealNtHeaders.address;
						...
						API(KERNEL32, LocalFree)(aRealNtHeaders.address);
				}
			}
...
BOOL kull_m_process_datadirectory(PKULL_M_MEMORY_ADDRESS pBase, DWORD entry, PDWORD pRva, PDWORD pSize, PWORD pMachine, PVOID *pData)
{
	...
	if(kull_m_process_ntheaders(pBase, &pNtHeaders))
	{
		...
		if(rva && size && pData)
		{
			if(*pData = API(KERNEL32, LocalAlloc)(LPTR, size))
			{
				...
				if(!status)
					API(KERNEL32, LocalFree)(*pData);
			}
		}
		API(KERNEL32, LocalFree)(pNtHeaders);
	}
	return status;
}
...
BOOL kull_m_process_create(KULL_M_PROCESS_CREATE_TYPE type, PCWSTR commandLine, DWORD processFlags, HANDLE hUserToken, DWORD logonFlags, PCWSTR user, PCWSTR domain, PCWSTR password, PPROCESS_INFORMATION pProcessInfos, BOOL autoCloseHandle)
{
	...
	ptrProcessInfos = pProcessInfos ? pProcessInfos : (PPROCESS_INFORMATION) API(KERNEL32, LocalAlloc)(LPTR, sizeof(PROCESS_INFORMATION));
	...
	if(dupCommandLine = _wcsdup(commandLine))
	{
		...
		{
			API(KERNEL32, CloseHandle)(ptrProcessInfos->hThread);
			API(KERNEL32, CloseHandle)(ptrProcessInfos->hProcess);
		}

		if(!pProcessInfos)
			API(KERNEL32, LocalFree)(ptrProcessInfos);
		free(dupCommandLine);
	}
	return status;
}
...
NTSTATUS kull_m_process_getExportedEntryInformations(PKULL_M_MEMORY_ADDRESS address, PKULL_M_EXPORTED_ENTRY_ENUM_CALLBACK callBack, PVOID pvArg)
{
	...
	if(kull_m_process_datadirectory(address, IMAGE_DIRECTORY_ENTRY_EXPORT, &rva, &size, &exportedEntry.machine, (PVOID *) &pExportDir))
	{
		...
		API(KERNEL32, LocalFree)(pExportDir);
	}
...
PSTR kull_m_process_getImportNameWithoutEnd(PKULL_M_MEMORY_ADDRESS base)
{
	...
	if(kull_m_memory_search(&aNullBuffer, sizeof(sEnd), &sMemory, FALSE))
	{
		...
		if(aStringBuffer.address = API(KERNEL32, LocalAlloc)(LPTR, size))
			if(!kull_m_memory_copy(&aStringBuffer, base, size))
				aStringBuffer.address = API(KERNEL32, LocalFree)(aStringBuffer.address);
	}
			...
			if(importedEntry.libname = kull_m_process_getImportNameWithoutEnd(&aProcName))
			{
					...
					)
				{
					...
					if(importedEntry.name)
						API(KERNEL32, LocalFree)(importedEntry.name);

				}
				API(KERNEL32, LocalFree)(importedEntry.libname);
			}
		}
		API(KERNEL32, LocalFree)(pLocalBuffer);
	}
	return TRUE;
}