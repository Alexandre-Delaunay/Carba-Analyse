...
void kull_m_asn1_append(PDIRTY_ASN1_SEQUENCE_EASY * parent, PDIRTY_ASN1_SEQUENCE_EASY child)
{
	...
	if(child)
	{
		...
		{
			if(buffer = (PDIRTY_ASN1_SEQUENCE_EASY) API(KERNEL32, LocalAlloc)(LPTR, szParent + szChild))
			{
		...
		else
		{
			...
			if(szTotal > 0x7f)
			{
				if(buffer = (PDIRTY_ASN1_SEQUENCE_EASY) API(KERNEL32, LocalAlloc)(LPTR, sizeof(DIRTY_ASN1_SEQUENCE_2) + szTotal))
				{
		...
		if(buffer)
		{
			API(KERNEL32, LocalFree)(child);
			API(KERNEL32, LocalFree)(*parent);
			*parent = buffer;
		}
	}
}
...
PDIRTY_ASN1_SEQUENCE_EASY kull_m_asn1_create(UCHAR type, LPCVOID data, DWORD size, PDIRTY_ASN1_SEQUENCE_EASY *parent)
{
	...
	if(size > 0x7f)
	{
		if(buffer = (PDIRTY_ASN1_SEQUENCE_EASY) API(KERNEL32, LocalAlloc)(LPTR, sizeof(DIRTY_ASN1_SEQUENCE_2) + size))
		{
	...
	{
		if(buffer = (PDIRTY_ASN1_SEQUENCE_EASY) API(KERNEL32, LocalAlloc)(LPTR, sizeof(DIRTY_ASN1_SEQUENCE_1) + size))
		{
